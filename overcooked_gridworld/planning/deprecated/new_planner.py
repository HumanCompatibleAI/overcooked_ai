...
import numpy as np
import pickle, time
from hr_coordination.utils import profile
from hr_coordination.planning.search import SearchTree, Graph, NotConnectedError
from hr_coordination.mdp.overcooked_mdp import Action, Direction, OvercookedState, PlayerState, OvercookedGridworld
...
class MotionPlanner(object):
    ...
    def min_cost_between_features(self, pos_list1, pos_list2):
        """
...
class JointMotionPlanner(object):
    ...
    def __init__(self, mdp, params, debug=False):
        ...
        self.orientations = True #params["orientations"]
    ...
    def _compute_plan_from_joint_graph(self, joint_start_state, joint_goal_state):
        ...
        if self.orientations:
            joint_pos_and_or_node_path = self.joint_graph_problem.get_node_path(joint_start_state, joint_goal_state)[1:]
            joint_positions_node_path = [tuple(player_pos_or[0] for player_pos_or in joint_player_pos_or) for joint_player_pos_or in joint_pos_and_or_node_path]
        else:
    ...
    def _joint_graph_from_grid(self):
        ...
        if self.orientations:
            valid_joint_positions_and_or = self.mdp.get_valid_joint_player_positions_and_orientations()
            for state_index, joint_pos_and_or in enumerate(valid_joint_positions_and_or):
                state_decoder[state_index] = joint_pos_and_or

            state_encoder = {v:k for k, v in state_decoder.items()}
            num_graph_nodes = len(state_decoder)

            adjacency_matrix = np.zeros((num_graph_nodes, num_graph_nodes))
            for start_state_index, start_joint_pos_and_or in state_decoder.items():
                start_positions = [item[0] for item in start_joint_pos_and_or]
                start_orientations = [item[1] for item in start_joint_pos_and_or]
                for joint_action, successor_jm_state in self._get_valid_successor_joint_positions(start_positions, start_orientations).items():
            ...
            for start_state_index, start_joint_positions in state_decoder.items():
                for joint_action, successor_joint_pos_and_or in self._get_valid_successor_joint_positions(start_joint_positions).items():
                    successor_positions, _ = [item[0] for item in successor_joint_pos_and_or]
                    # Successor positions is just the successor joint motion state
                    successor_jm_state = successor_positions
                    successor_node_index = state_encoder[successor_jm_state]
    ...
    def _get_valid_successor_joint_positions(self, starting_positions, starting_orientations=None):
        ...
        if starting_orientations is None:
            # Add dummy orientations
            # Under assumption that orientation doesn't matter
            starting_orientations = [Direction.NORTH, Direction.NORTH]
        
        dummy_player_states = [PlayerState(pos, orient) for pos, orient in zip(starting_positions, starting_orientations)]
        for joint_action in joint_motion_actions:
            new_positions, new_orientations = self.mdp.compute_new_positions_and_orientations(dummy_player_states, joint_action)
            successor_joint_positions[joint_action] = tuple(zip(new_positions, new_orientations))
...
class MediumLevelActionManager(object):
    ...
    def place_obj_on_counter_actions(self, state):
        all_empty_counters = set(self.mdp.get_empty_counters(state))
