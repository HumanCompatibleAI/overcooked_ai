...
class OvercookedState(object):
    ...
    # def __eq__(self, other):
    #     """
    #     Check equality between two states based on players' positions, orientations,
    #     held objects, and objects in the environment.
    #     """
    #     if not isinstance(other, OvercookedState):
    #         return False

    #     return self.players == other.players and self.objects == other.objects

    def __eq__(self, other):
        """
        Check equality between two states based on player positions,
        objects in the environment, and objects held by players.
        Excludes player orientations.
        """
        if not isinstance(other, OvercookedState):
            return False

        # Compare player states (position and held objects only)
        self_player_states = [
            (player.position, player.get_object().name if player.has_object() else None)
            for player in self.players
        ]
        other_player_states = [
            (player.position, player.get_object().name if player.has_object() else None)
        ...
        return (
            self_player_states == other_player_states and
    ...
    def __hash__(self):
        """
        Compute a unique hash for the state using player positions,
        objects in the environment, and objects held by players.
        Excludes player orientations.
        """
        # Players' positions and held objects (ignore orientation)
        player_states = tuple(
            (player.position, player.get_object().name if player.has_object() else None)
            for player in self.players
        )

        # Hash relevant components
        return hash((
            player_states,                    # Include player positions and held objects
            tuple(self.objects.items()),      # Objects in the environment
        ))
...
class OvercookedGridworld(object):
    ...
    def get_state_transition(
        ...
        ) = self.resolve_interacts(state, new_state, joint_action, events_infos)
        ...
        for player_idx, (player, action) in enumerate(
            ...
            print("Player {} took action {}".format(player_idx, action), "old position", state.players[player_idx].position, "new position", new_state.players[player_idx].position)
    ...
    def resolve_interacts(self, state, new_state, joint_action, events_infos):
        """
        ...
        for player_idx, (player, action) in enumerate(
            ...
            if terrain_type == "X":
                ...
                if self.is_ingredient_pickup_useful(state, pot_states, player_idx):
                    shaped_reward[player_idx] += self.reward_shaping_params[
                        "ONION_PICKUP_REWARD"
                    ]
                    print("Onion pickup is useful", shaped_reward[player_idx])
                player.set_object(obj)
    ...
    def calculate_distance_based_shaped_reward(self, state, new_state, player_idx):
        ...
        print("held object", player_new.held_object, "dishes in play", dishes_in_play, "nearly ready pots", len(nearly_ready_pots))
        ...
        if player_new.held_object is not None and player_new.held_object.name == 'onion' and len(nearly_ready_pots) == 0 and dishes_in_play == 0:
            print("checking nearest pot")
            min_dist_to_pot_new = np.inf
            min_dist_to_pot_old = np.inf
            for pot in self.terrain_pos_dict['P']:
                new_dist = np.linalg.norm(np.array(pot) - np.array(player_new.position))
                old_dist = np.linalg.norm(np.array(pot) - np.array(player_old.position))
                print(f"Player new position: {player_new.position}, Pot location: {pot}, New distance: {new_dist}")
                print(f"Player old position: {player_old.position}, Pot location: {pot}, Old distance: {old_dist}")
                if new_dist < min_dist_to_pot_new:
                    min_dist_to_pot_new = new_dist
                if old_dist < min_dist_to_pot_old:
                    min_dist_to_pot_old = old_dist
            if min_dist_to_pot_old > min_dist_to_pot_new:
                distance_based_shaped_reward += self.reward_shaping_params["POT_DISTANCE_REW"] * (1 - min(min_dist_to_pot_new / max_dist, 1))
                print("ADDED DISTANCE POT REWARD")
        ...
        if player_new.held_object is None and len(nearly_ready_pots) == 0 and dishes_in_play == 0:
            ...
            for onion_loc in self.terrain_pos_dict['O']:
                ...
                print(f"Player new position: {player_new.position}, Onion location: {onion_loc}, New distance: {new_dist}")
                print(f"Player old position: {player_old.position}, Onion location: {onion_loc}, Old distance: {old_dist}")
            ...
            if min_dist_to_onion_old > min_dist_to_onion_new:
                ...
                print("ADDED DISTANCE ONION REWARD")
