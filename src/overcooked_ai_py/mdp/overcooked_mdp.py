...
from overcooked_ai_py.utils import pos_distance, read_layout_dict, classproperty, weighted_random_by_dct, normalize
        ...
        def start_state_fn():
            ...
            for player in start_state.players:
                ...
                if p < player_litter:
                    ...
                    if obj == "soup":
                        player.set_object(
                            SoupState.get_soup(player.position, num_onions=n, finished=True)
                        )
                    else:
                        player.set_object(ObjectState(obj, player.position))
            return start_state
        ...
        for player_idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            if terrain_type == 'X':
                ...
                if self.soup_to_be_cooked_at_location(new_state, i_pos):
                    ...
                    # temporary modifying soup logic to keep "centre_pots", "bottleneck" bc in distribution:
                    if soup.is_full:
                        soup.begin_cooking()
