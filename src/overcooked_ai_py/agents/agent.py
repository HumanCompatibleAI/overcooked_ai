...
class ModelBasedAgent(Agent):
    ...
    def build_models(self, initial_state, max_horizon=1000):
        """
        Explore reachable states, construct transition and reward models,
        and propagate rewards backward through the trajectory.
        ...
        while queue and states_explored < max_horizon:
            state = queue.pop(0)
            state_hash = hash(state)  # Precompute the hash for efficiency
            ...
            # Track the trajectory of (state, action, reward)
            trajectory = []

            valid_joint_actions = self.mdp.get_actions(state)
            print(f"Valid joint actions: {valid_joint_actions}")
            print(f"Joint actions for agent {self.agent_index}: {valid_joint_actions[self.agent_index]}")
            
            for joint_action in itertools.product(*valid_joint_actions):
                print(f"Joint action: {joint_action}")

                # Get this agent's action
                agent_action = joint_action[self.agent_index]
                
                # Get the next state and reward
                next_state, infos = self.mdp.get_state_transition(state, joint_action)
                next_state_hash = hash(next_state)  # Compute hash for next_state

                # Compute reward for this joint action (agent-specific)
                reward = infos["sparse_reward_by_agent"][self.agent_index] + infos["shaped_reward_by_agent"][self.agent_index]

                # Update transition model
                self.transition_model[(state_hash, agent_action)][next_state_hash] += 1

                # Append to trajectory for backward reward propagation
                trajectory.append((state_hash, agent_action, reward))

                # If next_state is unexplored, add it to the queue
                if next_state_hash not in visited:
                    queue.append(next_state)

            # Back-propagate rewards through the trajectory
            cumulative_reward = 0
            for t in reversed(range(len(trajectory))):
                state_hash, agent_action, reward = trajectory[t]
                cumulative_reward = reward + self.discount_factor * cumulative_reward
                self.reward_model[(state_hash, agent_action)] += cumulative_reward

                # Print statement for debugging reward propagation
                print(
                    f"Back-propagating reward: {cumulative_reward} -> State: {state_hash}, Action: {agent_action}"
                )
        ...
        print("LENGTH OF TRANSITION MODEL", len(self.transition_model))

        # Save transition and reward models to text files
        self.save_transition_model_to_file()
        self.save_reward_model_to_file()

        self.models_built = True  # Mark models as built

    def save_transition_model_to_file(self, filename="transition_model.txt"):
        """
        Save the transition model to a text file.
        """
        with open(filename, "w") as f:
            f.write("Transition Model:\n")
            for (state_action, next_states) in self.transition_model.items():
                f.write(f"State-Action: {state_action}\n")
                for next_state, prob in next_states.items():
                    f.write(f"  -> Next State: {next_state}, Probability: {prob}\n")
                f.write("\n")  # Add a newline for separation
        print(f"Transition model saved to {filename}")

    def save_reward_model_to_file(self, filename="reward_model.txt"):
        """
        Save the reward model to a text file.
        """
        with open(filename, "w") as f:
            f.write("Reward Model:\n")
            for (state_action, reward) in self.reward_model.items():
                f.write(f"State-Action: {state_action}, Reward: {reward}\n")
            f.write("\n")  # Add a newline for separation
        print(f"Reward model saved to {filename}")

    def plan_value_function(self):
        """
        Use value iteration to compute the optimal value function.
        """
        all_states = {state_hash for state_hash, _ in self.transition_model.keys()}
        self.value_function = {state_hash: 0 for state_hash in all_states}
        ...
        for _ in range(100):  # Fixed number of iterations
            ...
            for state_hash in all_states:
                max_value = float('-inf')
                for action in Action.ALL_ACTIONS:
                    transitions = self.transition_model.get((state_hash, action), {})
                    reward = self.reward_model.get((state_hash, action), 0)
                    value = reward + self.discount_factor * sum(
                        prob * self.value_function.get(next_state_hash, 0)
                        for next_state_hash, prob in transitions.items()
                    )
                    max_value = max(max_value, value)
                new_value_function[state_hash] = max_value
    ...
    def action(self, state):
        ...
        for action in Action.ALL_ACTIONS:
            print(f"Action we are looking at: {action}")

            # check if state action pair exists in transition model
            if (state_hash, action) not in self.transition_model:
                print(f"State-Action pair: {state_hash, action} not in transition model")
                continue
            # check if state action pair exists in reward model
            if (state_hash, action) not in self.reward_model:
                print(f"State-Action pair: {state_hash, action} not in reward model")
                continue
            transitions = self.transition_model.get((state_hash, action), {})
            reward = self.reward_model.get((state_hash, action), 0)
