//it is render_event_chart.js file - do not delete this line, is it used for tests
    ...
    chart_box_view_box:undefined},)
{
    ...
    require(['d3', "jquery"], function(d3, jQuery) {
        ...
        var chart_box = d3.select(chart_box_id)
            .append("svg")
                .attr("class", "chart-box");

        if (typeof(settings.chart_box_view_box) !== "undefined"){
            chart_box.attr("viewBox", settings.chart_box_view_box);
        }

        var chart_area = chart_box.append("g")
        ...
        var xScaleTime = d3.scaleLinear()
            .range([0, inner_width])
            .domain([0, Math.max(max_end_timestep, max_timestep)]);  // resign with .nice() so chart timesteps can align with sliders of same width as chart area
