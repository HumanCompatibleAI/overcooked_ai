...
class TestBCTraining:
    """
    Tests for behavior cloning training and utilities
    ...
    min_performance (int):      Minimum reward achieved in BC-BC rollout after training to consider training successful
    """
    ...
    def test_save_and_load(self):
        ...
        assert self.bc_params == loaded_params
        assert np.allclose(model(self.dummy_input), loaded_model(self.dummy_input))
    ...
    def test_training(self):
        ...
        if self.strict:
            assert np.allclose(model(self.dummy_input), self.expected["test_training"])
    ...
    def test_agent_evaluation(self):
        ...
        assert results >= self.min_performance

        if self.compute_pickle:
            self.expected["test_agent_evaluation"] = results
        if self.strict:
            assert results == pytest.approx(self.expected["test_agent_evaluation"])
...
class TestBCTrainingLSTM(TestBCTraining):
    ...
    def test_lstm_evaluation(self):
        ...
        if self.strict:
            assert results == pytest.approx(self.expected["test_lstm_evaluation"])
